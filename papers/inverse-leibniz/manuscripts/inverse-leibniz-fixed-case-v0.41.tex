...
\author{Akari H.}
\date{Fixed-case completion candidate v0.41 --- August 2026}
...
The present draft establishes the following layers of the problem.
...
\begin{tabular}{@{}p{0.21\textwidth}p{0.25\textwidth}p{0.43\textwidth}@{}}
\toprule
Framework & Usual fixed data & Relation to the present problem \\
\midrule
...
Artin and DGLA deformation theory \cite{Schlessinger1968,GerstenhaberSchack1988} & A formal functor with tangent and obstruction spaces & The same language organizes our local equations, but the functor is a reconstruction fiber with $B$ held fixed. \\
Algebra morphisms and diagrams \cite{GerstenhaberSchack1983,GerstenhaberSchack1985} & Structure maps and their compatibility diagrams & The Leibniz equations are compatibility equations for actions, but no algebra morphism is being deformed unless extra data are added. \\
Module deformations \cite{Laudal2002} & An algebra together with a module or a finite module family & We keep $A$ and $B$ fixed and reconstruct the module actions themselves from the observed biderivation. \\
...
Double brackets and representation spaces \cite{VanDenBergh2008,BerestKhachatryanRamadoss2013} & A double operation or a representation scheme of a fixed algebra & The target is an arbitrary $V$ and its actions are unknown; the finite strict fiber is an action-reconstruction scheme, not a double-Poisson or derived-representation construction. \\
\bottomrule
\end{tabular}
\end{table}

Accordingly, the contribution claimed in this version is narrow and verifiable: a fixed cubic inverse-Leibniz example, its exact formal germ, the intrinsic higher-transfer and low-rail certificates, and the resulting finite spectral stratifications.  Whether the triangular mechanism extends as a genuinely new general theorem requires the abstract study postponed to later work.

\subsection*{Status and limitations}
This fixed-case completion candidate records the finite-dimensional reconstruction, obstruction, DGLA, finite-feedback, block-actuator, stationary-reuse, low-rail, residual-module, landing, and stratification results as one theorem package.  The calculations are checked by exact integer or rational symbolic computation, with modular minors used only as lower-bound certificates when paired with exact rational upper bounds.  The cubic example is verified through its complete formal germ, through full harmonic feedback at arity twenty-two, through the autonomous low-rail recursion at arity twenty-three, and through the all-orders autonomous low-rail theorem for the fixed contraction.  The order-fourteen transfer law has two frozen out-of-sample validations and is subsequently derived from the stabilized homogeneous operator and triangular forcing, rather than inferred from those data.  The landing and extension stratifications are exact for the canonical polynomial representatives in the fixed cubic $x_5$ module.  The supplied JSON certificates are part of the source bundle and are consumed by the portable verifiers described in Section~\ref{sec:exact-verification}.

The scope boundary is explicit.  This version makes no literature-level novelty claim and does not abstract the triangular spectral-extension mechanism to general contraction data or vary $A$ or $B$.  The fixed-$B$ gauge boundary is settled below: when $V=\Span B(A,A)$, the pointwise stabilizer is trivial, so the global four-point ambiguity is not a hidden conjugation artifact.  A future quotient theory is needed only for non-spanning observations or for simultaneous deformation of the underlying data.
...
For the fixed cubic data studied here, the inverse Leibniz program can now be closed at the level required for the next research step.  The paper has established the following finite package:
\begin{enumerate}[label=(\roman*)]
...
\item the dual-number and cubic obstruction calculations, including the exact germ $\C[[u,v]]/(v^3)$ and the intrinsic constrained Hochschild transfer through arity seven;
\item the finite-feedback, actuator, stationary-reuse, and autonomous low-rail results, with frozen OOS certificates at $Y_{17}$ and $Y_{18}$;
...
\end{enumerate}
The fixed-$B$ gauge boundary is also explicit: in the spanning examples the pointwise stabilizer of $B(A,A)$ is trivial, so the four-point ambiguity is a genuine reconstruction ambiguity.  The accompanying JSON records and portable verifier suite make the finite certificate chain executable from this directory.

The general problem with spectral multiplicity data $m_A(S)=\prod_\lambda(S-\lambda)^{e_\lambda}$ begins only after this fixed-case package.  Its forcing-side hypotheses, exponent-truncation rules, and gauge-compatible formulation are not asserted here; they are the subject of the open problem in Section~\ref{sec:open-problems}.
...
\begin{problem}[Gauge quotient beyond the spanning case]
Extend the fixed-$B$ action of Proposition~\ref{prop:fixed-B-gauge} to a degree-zero intrinsic DGLA or Deligne groupoid when $B(A,A)$ does not span $V$.  Compare framed rigidity with rigidity modulo the resulting stabilizer, and then allow simultaneous deformations of $A$ or $B$.
\end{problem}
...
Extend the bounded comparison in Section~\ref{sec:literature-position} into a systematic survey of universal differential calculi, Hochschild cohomology, double derivations, representation schemes, and deformation theory of modules and algebra homomorphisms.  The present paper makes only the fixed-case claim stated there; any broader novelty claim requires that survey.
\end{problem}
...
The frozen out-of-sample records are distributed as two JSON certificates.  The v0.32 certificate stores the $Y_{17}$ prediction, its independently computed arity-$22$ value, the four railwise equality flags, and the arity-$22$ controller-rank checkpoints.  The v0.33 certificate stores the $Y_{18}$ prediction and independent arity-$23$ value together with the low-rail histories, the source reconstruction, homotopy basis, cyclic matrix $A_*$, and residual-polynomial data.  The v0.34 and v0.36 verifiers read the latter file from their own directory by default and accept an explicit path through their \texttt{--certificate} option; this removes the former machine-specific \texttt{/mnt/data} dependency.  The v0.32 record is audited for railwise equality and internal rank-shape consistency by the companion v0.32 verifier.  These checks establish exact certificate consistency; they do not turn the fixed-case statements into an abstract theorem for general triangular systems.
...
\href{https://doi.org/10.1007/978-94-009-3057-5}{doi:10.1007/978-94-009-3057-5}.
...
Y.~Berest, A.~Khachatryan, and A.~Ramadoss,
\emph{Derived representation schemes and noncommutative geometry},
