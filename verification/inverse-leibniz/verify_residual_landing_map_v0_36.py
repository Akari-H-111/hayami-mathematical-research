...
parser = argparse.ArgumentParser(description="Verify the v0.33 low-rail certificate.")
parser.add_argument(
    "--certificate",
    type=Path,
...
# ===========================================================================
# v0.36 residual landing map: finite-jet spectral selection
# ===========================================================================

# The actual cubic landing vectors are finite-jet quantities:
#   Lambda4 = p3(S)x4(0)
#   Lambda5 = p4(S)x5(0).
# No long temporal orbit is needed to determine the multiplier.

LAMBDA4_ACT = y4[0]
LAMBDA5_ACT = y5[0]

# E-extension certificate for x4.
assert flat_c1(LAMBDA4_ACT)[((0,1),(0,0))] == Fraction(-3200)
assert Aop(LAMBDA4_ACT) == c1_scale(LAMBDA4_ACT,Fraction(-2))

# Full spectral extension certificate for x5.
KRYLOV5=[LAMBDA5_ACT]
for _ in range(5):
    KRYLOV5.append(Aop(KRYLOV5[-1]))

keys36=sorted(set().union(*[set(flat_c1(v)) for v in KRYLOV5]))
K36=sp.Matrix([
    [sp.Rational(flat_c1(v).get(key,Fraction(0))) for v in KRYLOV5]
    for key in keys36
])
assert K36.rank() == 6
pivot_rows36=K36.T.rref()[1]
assert len(pivot_rows36)==6
DET_LANDING5=sp.factor(K36[list(pivot_rows36),:].det())
assert DET_LANDING5 == 68852103320239427215687680

print("v0.36 residual landing map: PASS")
print("Lambda4(actual) is nonzero and lies in ker(A+2I)")
print("Lambda5(actual) is A-cyclic, Krylov rank 6")
print("Lambda5 Krylov minor det =",DET_LANDING5)
