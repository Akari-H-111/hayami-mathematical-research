...
def check(record):
    ...
    with tempfile.TemporaryDirectory(prefix=f'paper_{roman}_submission_') as temp:
        ...
        old=ROOT.parent / ({'I': 'paper-01', 'II': 'paper-02', 'III': 'paper-03'}[roman] + '-illustrated-v0.09') / (stem+'.pdf')
        for tag,input_pdf in [('old',old),('new',pdf)]:
            out=work/tag;out.mkdir()
            run(['pdftoppm','-r','90','-png',str(input_pdf),str(out/'page')])
        changed=[]
        for previous in sorted((work/'old').glob('*.png')):
            current=work/'new'/previous.name
            if sha(previous)!=sha(current):
                n=int(previous.stem.split('-')[-1]);changed.append(n)
        ...
        assert all(n>=record['pages']-1 for n in changed),changed
        out=ROOT/'pdf';out.mkdir(exist_ok=True)
        shutil.copyfile(pdf,out/pdf.name)
        record.update({'pdf_sha256':sha(pdf),'extracted_compile':'PASS (Tectonic)',
            'qpdf':'PASS','figure_destinations':'PASS','changed_rendered_pages':changed,
            'unchanged_rendered_pages':record['pages']-len(changed),
