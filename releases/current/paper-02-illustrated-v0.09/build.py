...
parser.add_argument('--figures', action='store_true', help='Rebuild the seven vector masters and exports')
...
if args.paper:
    subprocess.run(['tectonic', '--keep-logs', 'paper_II_marked_naturality_v0_09.tex'], cwd=ROOT, check=True)
if args.render:
    ...
    subprocess.run(['pdftoppm', '-r', '120', '-png', str(ROOT / 'paper_II_marked_naturality_v0_09.pdf'), str(destination / 'page')], check=True)
