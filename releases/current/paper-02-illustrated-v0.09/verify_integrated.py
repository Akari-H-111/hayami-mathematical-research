...
original = (ROOT / 'provenance/paper_II_marked_naturality_v0_08.tex').read_text()
...
restored = restored.replace('Paper II, illustrated edition v0.09', 'Paper II, manuscript closeout v0.08')
...
assert text.count(r'\begin{figure}') == text.count(r'\end{figure}') == 7
assert text.count(r'\includegraphics') == 7
...
assert sorted(destinations) == list(range(1, 8))
assert list(destinations.values()) == sorted(destinations.values())
captions = {}
with pdfplumber.open(ROOT / (STEM + '.pdf')) as document:
    for n, page in enumerate(document.pages, 1):
        for number in re.findall(r'^Figure\s+(\d+)\.', page.extract_text() or '', re.M):
...
qa.update({'body_preserved_except_author_and_marked_editorial_additions': True,
...
print(f"PASS: {qa['pages']} pages, seven vector figures, six bibliography entries, exact mathematical body preservation, author, citations, destinations, clean manuscript log, bounds and evidence hashes.")
