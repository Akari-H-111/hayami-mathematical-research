...
\date{Paper I, illustrated edition v0.09 --- September 2026}
\hypersetup{pdftitle={The Inverse Leibniz Problem},pdfauthor={Akari Hayami (Jian-Yu Huang)},
...
The overall reading map is shown in Figure~\ref{fig:paperI-01}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_01_results_roadmap.pdf}}
\caption{The five result groups of Paper~I. The rows give a reading order, not additional implication claims. The transfer endpoint uses one compatible contraction; the spectral calculation retains the chosen low-rail homotopy.}
\label{fig:paperI-01}
\end{figure}
% END ILLUSTRATION 1
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-02}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_02_reconstruction_fibers.pdf}}
\caption{Weak and strict reconstruction for fixed $B$. The nonempty weak fiber is affine, while strictness imposes the unital bimodule equations. The fixed-$B$ stabilizer is trivial when $B(A,A)$ spans $V$; strict tangent observability is described by $\ker\cD_p$. Nested regions are schematic, not dimension estimates.}
\label{fig:paperI-02}
\end{figure}
% END ILLUSTRATION 2
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-03}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_03_four_rigid_points.pdf}}
\caption{The four strict reconstructions of Theorem~\ref{thm:four-points}. Independent swaps of evaluation points preserve $B$. Each point is reduced and infinitesimally rigid, but the fixed-$B$ fiber is not globally unique. The arrows are swaps of the displayed constructions, not gauge identifications.}
\label{fig:paperI-03}
\end{figure}
% END ILLUSTRATION 3
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-04}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_04_dual_number_tangent.pdf}}
\caption{The dual-number false tangent of Theorem~\ref{thm:false-tangent}. The halo denotes nilpotent structure at one geometric point; the dashed line denotes the Zariski tangent direction, not a family of geometric points. Every nonzero first-order direction is obstructed at second order.}
\label{fig:paperI-04}
\end{figure}
% END ILLUSTRATION 4
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-07}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_07_dgla_comparison.pdf}}
\caption{The relation DGLA morphism and its induced commuting obstruction square (Proposition~\ref{prop:relation-dgla-morphism} and Corollary~\ref{cor:cohomological-projection}). The map on tangent spaces is the identity; the degree-two map projects intrinsic obstruction classes to presentation relations. No quasi-isomorphism is asserted.}
\label{fig:paperI-07}
\end{figure}
% END ILLUSTRATION 7
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-08}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_08_effective_obstruction_chain.pdf}}
\caption{The effective obstruction comparison chain. The dimension of $(I_\kappa/\mathfrak nI_\kappa)^\vee$ counts minimal formal equations. Its displayed injection into $H^2$ depends on the minimal-model realization, and its composite with $\overline\Pi$ need not be injective. Unused relation directions and the kernel of the composite are different phenomena.}
\label{fig:paperI-08}
\end{figure}
% END ILLUSTRATION 8
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-05}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_05_mixed_vs_cubic.pdf}}
\caption{The mixed scheme $\Spec\C[u,v]/(uv,v^2)$ and the cubic formal germ $\C[[u,v]]/(v^3)$ have different transverse nilpotent structures. The first has an embedded false direction only at the origin; the second has uniform cubic thickness. Shading is schematic and does not represent additional branches.}
\label{fig:paperI-05}
\end{figure}
% END ILLUSTRATION 5
...
The corresponding structures are illustrated in Figure~\ref{fig:paperI-06}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_06_finite_flat_family.pdf}}
\caption{The finite flat family of Proposition~\ref{prop:cubic-flat-family}. Write $q_u=1+2u$, distinct from the later quintic $q(S)$. Every fiber has length three. At $u=-1/2$ its embedding dimension rises from one to two. The arrows depict multiplication on bases, not distinct geometric points.}
\label{fig:paperI-06}
\end{figure}
% END ILLUSTRATION 6
...
The two completion endpoints are compared in Figure~\ref{fig:paperI-09}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_09_arity_branching.pdf}}
\caption{Arity comparison for two completions of the saved partial homotopy. The frozen completion has a held-out nonzero arity-$23$ coefficient (Proposition~\ref{prop:frozen-23}); a different compatible completion controls arity~$23$ to zero (Theorem~\ref{thm:compatible-23}). Both have the same cubic formal germ. Neither result asserts vanishing at arity~$24$ or above.}
\label{fig:paperI-09}
\end{figure}
% END ILLUSTRATION 9
...
% BEGIN ILLUSTRATION 12
The corresponding structures are illustrated in Figure~\ref{fig:paperI-12}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_12_spectral_extension_tower.pdf}}
\caption{The triangular spectral extension and residual-module tower. With $E=S+2$ and $m_A=Eq$, the successive multipliers are $E,E,m_A$. The residual modules have dimensions $1,2,8$. The annihilators are exact all-orders conclusions for the fixed autonomous subsystem, not contraction-independent invariants.}
\label{fig:paperI-12}
\end{figure}
% END ILLUSTRATION 12
...
% BEGIN ILLUSTRATION 13
The corresponding structures are illustrated in Figure~\ref{fig:paperI-13}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_13_landing_operators.pdf}}
\caption{Complete landing operators and spectral selection. The rank-one map $\Lambda_4$ lands in the resonant eigenline; $\Lambda_5$ is onto the six-dimensional homogeneous module and has a two-dimensional kernel. The finite-jet formula assumes $P(S)f=0$. In the fixed cyclic module, the surviving homogeneous annihilator is $m_A/\gcd(m_A,r_d)$.}
\label{fig:paperI-13}
\end{figure}
% END ILLUSTRATION 13
...
% BEGIN ILLUSTRATION 14
The corresponding structures are illustrated in Figure~\ref{fig:paperI-14}.
\begin{figure}[tbp]
\centering
\makebox[\textwidth][c]{\includegraphics[width=7.0in]{figures/fig_14_three_level_stratification.pdf}}
\caption{Three levels of spectral contact. The left diagram is an eight-vertex slice of the two-axis closure order, with all other quintic roots excluded from $Z$; arrows point toward additional vanishing. The full splitting-field counts are $64$, $128$, and $708$ nonzero fine strata plus the zero datum. The third level uses the specified degree-$<8$ polynomial lift, not an $\C[S]$-linear splitting.}
\label{fig:paperI-14}
\end{figure}
% END ILLUSTRATION 14
...
entry point. Accompanying collection: \href{https://doi.org/10.5281/zenodo.22663942}{doi:10.5281/zenodo.22663942}.
...
Paper II, illustrated edition v0.09, 2026. Companion manuscript in the accompanying collection.

\bibitem{PaperIII}
Akari Hayami (Jian-Yu Huang),
\emph{General Spectral Floors from Marked Deformation Presentations},
Paper III, illustrated edition v0.09, 2026. Companion manuscript in the accompanying collection.
\end{thebibliography}
