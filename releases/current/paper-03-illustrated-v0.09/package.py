...
ARCHIVE = ROOT.parent / 'paper_III_illustrated_v0_09_source.zip'
RECEIPT = ROOT.parent / 'paper_III_illustrated_v0_09_receipt.json'
...
with tempfile.TemporaryDirectory(prefix='paper-III-v009-portability-') as temporary:
    temp = Path(temporary)
    with zipfile.ZipFile(ARCHIVE) as archive:
    ...
    # Compile in a fresh directory using only final TeX and the ten vector PDFs.
    build = temp / 'compile'
    ...
    print('PASS all ten native figure rebuilds pixel-identical', flush=True)
receipt = {'date': '2026-09-08', 'archive': ARCHIVE.name, 'sha256': sha(ARCHIVE),
           ...
           'payload_hashes_checked': count, 'pages': qa['pages'], 'figures': 10,
           ...
           'all_rebuilt_pages_pixel_identical': True, 'all_10_native_figures_pixel_identical': True,
           'originals_unchanged': True,
           'scope': 'Paper III illustrated closeout and unchanged finite-evidence replay. Paper I high-arity v0.05/v0.06 completion replays are not repeated in this delivery.',
